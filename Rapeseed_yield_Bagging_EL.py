...
from sklearn.base import BaseEstimator, RegressorMixin  # 为自定义 Bagging 包装器服务
from matplotlib.ticker import FuncFormatter


# Constructing the Bagging
class BaggingEnsemble(BaseEstimator, RegressorMixin):
    def __init__(self, estimators, weights=None, name="Bagging"):
...
# —— Bagging weighted average-EL —— #
r2_list = np.array([metrics_RF['R2'], metrics_LGBM['R2'], metrics_XGB['R2']], dtype=float)
r2_nonneg = np.clip(r2_list, a_min=0.0, a_max=None)
if r2_nonneg.sum() <= 1e-12:
    weights = np.ones_like(r2_nonneg) / len(r2_nonneg)
else:
    weights = r2_nonneg / r2_nonneg.sum()
print(f"Weight of bese-models: RF={weights[0]:.3f}, LGBM={weights[1]:.3f}, XGB={weights[2]:.3f}")

bag_weighted = BaggingEnsemble([forest, LGBM, XGB_model], weights=weights, name="Bagging-Weighted").fit(X_train, Y_train)
y_pred_bag_weighted = bag_weighted.predict(X_validation)
metrics_bag_weighted = evaluate_model("Bagging(Weighted by R²)", Y_validation, y_pred_bag_weighted)
...
print_metrics("Bagging(R²加权)", metrics_bag_weighted)
...
# Partial dependence plots of the predictors in the bagging
def plot_pdp(X_train, model, features):
    ...
    plt.suptitle(f"PDP - {getattr(model, 'name', 'Bagging')}", fontsize=18, weight='bold', y=1.02)
    plt.tight_layout()
    plt.show()
plot_pdp(X_train, bag_weighted, X_train.columns)
...
def plot_density_scatter(y_true, y_pred, model_name="Model", threshold_percentile=95):
    ...
    plt.tight_layout()
    plt.show()
plot_density_scatter(Y_validation, y_pred_bag_weighted, model_name="Bagging-WAE")
bag_weighted = BaggingEnsemble([forest, LGBM, XGB_model], weights=weights).fit(X_train, Y_train)
plot_pdp(X_train, bag_weighted, X_train.columns)
...
for y in years:
    ...
    try:
        ...
        # Predict by using bagging-EL
        y_pred = bag_weighted.predict(X_2d)
        out_file = os.path.join(output_path, f'predict{y}_WAE.tif')
